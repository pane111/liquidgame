...
func _ready() -> void:
	...
	load_new_area(LocationManager.get_area("boss_office"),false)
	set_evidence()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if Input.is_action_just_pressed("number") && teleportationAllowed:
		match event.keycode:
			KEY_0:
				load_new_area(LocationManager.get_area("test"))
			KEY_1:
				load_new_area(LocationManager.get_area("entrance_hall"))
			KEY_2:
				load_new_area(LocationManager.get_area("office"))
			KEY_3:
				load_new_area(LocationManager.get_area("boss_office"))
			KEY_4:
				load_new_area(LocationManager.get_area("staircase"))
			KEY_5:
				load_new_area(LocationManager.get_area("basement_hallway"))
				AudioManager._stop_music()
			KEY_6:
				load_new_area(LocationManager.get_area("cooling_room"))
			KEY_7:
				load_new_area(LocationManager.get_area("fabricator_room"))
			KEY_8:
				load_new_area(LocationManager.get_area("roof"))
			KEY_9:
				load_new_area(LocationManager.get_area("break_room"))
func load_path(area: String, fade=true):
	load_new_area(LocationManager.get_area(area),fade)
